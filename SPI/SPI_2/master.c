...
int main() {
	...
	DS13xx_Write_CLK_Registers();
		 ...
		 // The Register will be read every Sec=1000ms
    ...
    while (1) {
		...
		datafromslave = SPI_MasterTransmit('B');//buffer[0]
		SS_UNSELECT
        uart_transmit_string("Gesendet\n\r");
       // uart_transmit_string("Empfangen: ");
        uart_transmit(datafromslave);
        uart_transmit_string("\r\n");
        SS_SELECT
        datafromslave = SPI_MasterTransmit('A'); //buffer[1]
		SS_UNSELECT
        uart_transmit_string("Gesendet\n\r");
       // uart_transmit_string("Empfangen: ");
        uart_transmit(datafromslave);
